...
## Design system — Ridley Academy UI (v633+, prototype on webinars.html)

The dashboards' identity lives in the design-system artifact **"Ridley Academy"** (https://claude.ai/artifact/B4s3s7Mdigr3wzQ46e3sRu): tokens, `ra-` components, usage rules, and the Webinars reference page. Built from the official brand guide (Brand Red #AC1818, Near-Black #1C1A18, Paper #F2EFE9, matte Gold #BFA050). Font: **Montserrat everywhere** since v649 (was Playfair Display + Inter); load `Montserrat:wght@400;500;600;700;800;900` and read chart fonts from `--font-sans`. Weights: body 500, small secondary lines 600, chart ticks 600 / point labels 700; never `-webkit-font-smoothing: antialiased` (it thins light-on-dark text).
...
- Secondary text must stay easy to read (owner, v645/v646): text in Stage is pure white (`ink`, `ink-muted` #FFFFFF, `ink-faint` #FFFFFF, chart line `viz-1` #FFFFFF — never warm/beige) and pure black #000000 in Paper (`ink`, `ink-muted`, `ink-faint`, `viz-1`). Don't reintroduce soft greys for labels, sub-lines or chart text.
- Pages on it so far: webinars.html (v633) and weekly-stats.html (v641); the shared `.topbar` (mobile.css) is unchanged until all pages move over.
- weekly-stats.html keeps its own inline topbar/login CSS (it does not load mobile.css — adding it clashes with a bespoke topbar); its old variable names (`--surface2`, `--text-dim`, `--green`, `--red`…) are mapped onto the tokens at the top of its `<style>`. Chart colours come from tokens via `tok('--name')` at draw time; wrong-way line segments use the page token `--ws-chart-down` (a clear red the owner asked for). Deltas: arrow = direction, colour = good/bad, `invert_chart` flips the colour only (the y-axis is no longer reversed). Charts are 2 per row. "▶ Present" opens a click-only full-screen presentation mode (one chart at a time, current tab's order). The script tag is `weekly-stats.js?v=N` — bump N whenever weekly-stats.js changes, or browsers mix new HTML with the cached old script.
...
- Where registrants came from (utm) before a webinar: read from AXL `webinar-user` by webinar-analytics (summary + cron job 40). Don't trust the thank-you page report for sources — it misses most people. After the webinar the roster (`webinar_source_stats`) is the source.
