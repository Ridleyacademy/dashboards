...
**What:** Rex's and Madison's entire consolidated rooms were deleted on Zoom (both return Zoom code 3001; verified via a throwaway `zoom-check` probe — Carlos/Ricardo/Dan rooms still alive). Root cause is NOT manual Zoom deletion — it's the `cancel` action: it ran `zoom DELETE /meetings/{zid}` unconditionally, but in the one-room-per-coach model many class rows share ONE zoom_meeting_id, so cancelling ONE class (Rex's Friday row 125; Madison's Monday/Friday-makeup rows 178/179) deleted the WHOLE Zoom room → the coach's other classes (still status='scheduled') silently went dead. No cancellation emails fired because those rows had 0 `email_sent` registrants (consolidated rows track email_sent elsewhere), so it was silent — "no log." Fix: `zoom-meetings` v48 (deploy byte-verified, sha e9a6ee94) — SHARED-ROOM GUARD: before the Zoom DELETE, count other scheduled rows on the same zoom_meeting_id; if any exist, DO NOT delete the Zoom meeting (and skip cancelScheduledReminders) — just mark this one row cancelled. Only the last scheduled row on a meeting id triggers the actual Zoom delete. Response now returns `zoom_meeting_deleted` + `shared_room_preserved`.
**Why:** A normal "cancel this one class" must never destroy a coach's shared room + all their other classes.
**Touched:** zoom-meetings v48 (backend only). Recovery of the already-deleted Rex (81434169178) + Madison (83187986921) rooms still pending — best path is restoring from Zoom "Recently Deleted" (~7-day window) which preserves links + registrations; else rebuild + re-invite (emails students, needs go-ahead). Throwaway `zoom-check` fn left as an inert 410 stub (no MCP delete tool available).
